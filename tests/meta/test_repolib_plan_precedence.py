...
def test_universal_doc_ships_to_all_types(tmp_path: pathlib.Path) -> None:
	...
	plan_other = repolib.plan.compute_propagation_plan(str(tmp_path), 'other')
...
#============================================
# PyPI child-type routing
#============================================

def _build_pypi_overlay(template_root: pathlib.Path) -> None:
	"""Create a synthetic templates/pypi overlay with devel + noexist files."""
	pypi = template_root / 'templates' / 'pypi'
...
def test_pypi_type_ships_publishing_tool(tmp_path: pathlib.Path) -> None:
	"""The pypi child type ships its publishing tool."""
	_build_pypi_overlay(tmp_path)
	plan = repolib.plan.compute_propagation_plan(str(tmp_path), 'pypi')
	assert 'submit_to_pypi.py' in plan['devel_files']


def test_pypi_noexist_routes_to_noexist(tmp_path: pathlib.Path) -> None:
	"""The PyPI seed lands in noexist rather than overwrite."""
	_build_pypi_overlay(tmp_path)
	plan = repolib.plan.compute_propagation_plan(str(tmp_path), 'pypi')
...
def test_python_type_does_not_ship_pypi_content(tmp_path: pathlib.Path) -> None:
	"""A plain python type stays free of PyPI-only content."""
	_build_pypi_overlay(tmp_path)
	repo_dir = tmp_path / 'consumer'
	repo_dir.mkdir()
	(repo_dir / 'pyproject.toml').write_text('test')
	plan = repolib.plan.compute_propagation_plan(str(tmp_path), 'python', repo_dir=str(repo_dir))
	assert 'submit_to_pypi.py' not in plan['devel_files']
	assert 'pyproject.toml' not in plan['noexist_files']


def test_pypi_content_stays_out_of_unrelated_types(tmp_path: pathlib.Path) -> None:
	"""PyPI-only content stays out of unrelated type chains."""
	_build_pypi_overlay(tmp_path)
...
def test_exclude_repos_blocks_named_consumer(tmp_path: pathlib.Path, monkeypatch: pytest.MonkeyPatch) -> None:
	...
	normal_repo.mkdir()
	excluded_repo.mkdir()
	plan_normal = repolib.plan.compute_propagation_plan(str(tmp_path), 'python', repo_dir=str(normal_repo))
	plan_excluded = repolib.plan.compute_propagation_plan(str(tmp_path), 'python', repo_dir=str(excluded_repo))
